...
sub run_test
{
    ...
    {
        my $gitimeout = int(($max_time*1.2*3)/1000)+1;
        if($gitimeout<2){$gitimeout=2;}
        `rm -f $dir/file.txt ; cp $test_dir/$file.txt $dir/file.txt`;
        `rm -f $dir/file.png ; cp $test_dir/$file.png $dir/file.png`;
        `cd $dir ; gitimeout $gitimeout ./minigl-nogl -i file.txt -s file.png -o $token.txt -n 3 || echo TIMEOUT >> $token.txt`;
